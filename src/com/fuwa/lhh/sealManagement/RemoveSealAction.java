...
@Service
public class RemoveSealAction extends ActionSupport
{
		...
		throws Exception
	{
		...
		{
			success = true;
		/*	for(FWUser u:fwUsers) {
				if(u.getUuid().equals(fwUser.getUuid())) {
					message = "该部门已经存在该用户,请重新填写用户名称!";
					success = false;
					break;
				}
			}*/
			...
			if(success) {
				...
				//fwUser.getGroups().add(currentDepart);
				//organizationDataManager.modifyUser(fwUser);
				
				
				//currentDepart.getUsers().add(fwUser);
				//organizationDataManager.modifyDepart(currentDepart);
				tm = new TreeModel();
