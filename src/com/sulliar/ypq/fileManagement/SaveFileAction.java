...
public class SaveFileAction extends ActionSupport
{
		...
		throws Exception
	{
		...
		if (sourceFile.exists())
		{
			...
			Department depart = departManager.getPersonDepartByUserId(user.getUuid());
			System.out.println("用户组别:"+depart.getDepartment_name());
			...
			System.out.println(toString());
			path = fileManager.DownloadFile(sourceFile, URLDecoder.decode(dataset_type, "utf-8"), realpath, stampList, user.getUser_name(), URLDecoder.decode(wp, "utf-8"),depart.getDepartment_name(),false);
			System.out.print("保存成功");
			flag = true;
			success = true;
			//User tlg = userDataManager.findUserByName(user.getUser_name());
			FWLog log = new FWLog();
			log.setUser(user);
	...
	public String toString() {
		return "SaveFileAction [nodePath=" + nodePath + ", filePath="
				+ filePath + ", flag=" + flag + ", success=" + success
				+ ", dataset_type=" + dataset_type + ", path=" + path
				+ ", waterm_name=" + waterm_name + ", item_id=" + item_id
				+ ", searchType=" + searchType + ", item_name=" + item_name
				+ ", item_rev=" + item_rev + ", wp=" + wp + ", overDownload="
				+ overDownload + ", logManager=" + logManager
				+ ", fileManager=" + fileManager + ", recordManager="
				+ recordManager + ", stampManager=" + stampManager + "]";
	}
