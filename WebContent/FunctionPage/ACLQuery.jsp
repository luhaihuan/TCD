...
Ext.onReady(function () {
	...
	var yn = Ext.create('Ext.data.Store', {
 	...
 	var storts=	 Ext.create('Ext.data.Store', {
 		storeId : 'aclStore',
 		fields : [ 'siteStr','groupStr','username','typeStr','printWord','downloadWord','printExcel','downloadExcel','printPDF','downloadPDF','bomSearch','materialSearch','parentSearch' ],
 		model : 'ACLTree',
 		pageSize: 10,
 		proxy : {
 			type : 'ajax',
 			url : 'GetACLbyDepartIdAndUserId', 			
 			reader : {
 				type : 'json',
	...
	Ext.create('Ext.container.Viewport', {
		...
		items : [ {
					...
					items : [{
						...
						text: '查看权限规则',
						handler:function(){
										var siteId = Ext.getCmp("SiteCombo").getValue();
								var departId = Ext.getCmp("GroupCombo").getValue();
								var userId = Ext.getCmp("UserCombo").getValue();
	  	        	//alert(ftcombo.getValue());
	  	            var params = {  
	  	                //参数  
	  	               siteId:encodeURIComponent(siteId),
	  	             	departId:encodeURIComponent(departId),
